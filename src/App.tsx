...
function Layout({ children }: { children: ReactNode }) {
  ...
  return <div className="app"><header><Link className="brand" to="/" aria-label="Студент ИУ5 — главная"><img src="/logo-iu5.jpeg" alt="Логотип Студент ИУ5" />Студент ИУ5</Link><nav aria-label="Основная навигация"><Link to="/">Каталог</Link><Link to="/search">Поиск</Link>{admin && <Link to="/admin/stats">Статистика</Link>}</nav></header>{user && location.pathname === '/' && <p className="user-greeting">Привет, {user.firstName}</p>}<main key={location.pathname}>{location.pathname !== '/' && !location.pathname.startsWith('/course/') && !location.pathname.startsWith('/admin/') && <button className="in-app-back" onClick={() => goBack(navigate)}>← Назад</button>}{children}</main><footer>Студент ИУ5 · Материалы открываются на Яндекс.Диске</footer></div>
}
function Home() {
