...
function CoursePage() {
  ...
  useEffect(() => {
    void repository.getFolder(id, path).then((result) => { setItems(result); setError('') }).catch((reason: unknown) => setError(reason instanceof Error ? reason.message : 'Не удалось загрузить каталог.')).finally(() => setLoading(false))
  }, [id, path])
  const downloadFile = (item: DiskItem) => { void repository.getFileUrl(id, item.path).then((url) => downloadExternalFile(url, item.name)).catch((reason: unknown) => setError(reason instanceof Error ? reason.message : 'Не удалось скачать файл.')) }
  const catalogItems = items.map((item) => ({ item, semester: !path && item.type === 'dir' ? semesterFromFolderName(item.name) : undefined }))
