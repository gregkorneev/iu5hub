...
- Навигационная группа — эталон для стеклянных controls: одна общая capsule-поверхность содержит обычные прозрачные ссылки; единственный selection bubble является псевдоэлементом общей поверхности и перемещается через `transform`. Отдельных рамок/материалов у каждой ссылки нет. В мобильной компоновке capsule центрируется под брендом и не растягивается сверх необходимого; ссылки сохраняют hit area не меньше 44×44 px. Для движения действует глобальный `prefers-reduced-motion`. Этот визуальный приём не меняет ссылок, порядка и navigation state.
