...
- Нижняя навигация — отдельный persistent functional layer над содержимым, не часть header или footer. Glass-капсула содержит три route links Каталог / Поиск / Профиль для всех ролей; Статистика доступна только подтверждённым администраторам отдельной ссылкой в правой части шапки и не занимает место вкладки. Если текущий экран уже имел действия «Назад» или «К корню курса», они отображаются отдельными компактными capsule controls над tab bar, с промежутком между слоями; маршруты и обработчики сохранены. Selection bubble принадлежит только трём route links. Bar ограничен шириной экрана и центрируется, а не растягивается на desktop; внешний край и bubble используют capsule radius. При минимальной hit area 44 px bubble высотой 36 px оставляет равный вертикальный стеклянный зазор сверху и снизу. `aria-current="page"` сообщает выбранный destination независимо от bubble. Для движения действует глобальный `prefers-reduced-motion`.
