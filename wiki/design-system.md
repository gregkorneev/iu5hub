...
- Нижняя навигация — отдельный persistent functional layer над содержимым, не часть header или footer. Единая glass capsule содержит доступные пользователю route links; один selection bubble принадлежит поверхности и перемещается через `transform`. CSS grid создаёт равные сегменты по фактическому числу ссылок (2 для обычного пользователя, 3 для подтверждённого admin), поэтому индикатор перестраивается при смене числа пунктов и viewport без фиксированных pixel offsets. Контейнер ограничен шириной экрана и центрируется, а не растягивается на desktop. Ссылки сохраняют hit area не меньше 44×44 px; `aria-current="page"` сообщает выбранный destination независимо от bubble. Для движения действует глобальный `prefers-reduced-motion`.
...
**Навигационный инвариант:** сохранены `HashRouter`, все существующие маршруты и иерархия каталога. Нижние tabs являются только верхнеуровневыми destinations и сохраняют последнюю route в Catalog/Search, пока приложение смонтировано; Search-result provenance следует за внутренними папками, а прямые ссылки разрешаются по реальному URL. Back и Telegram BackButton по-прежнему используют `goBack`, отдельно от переключения вкладок. Подробности route matrix, role visibility и Apple reference см. `telegram.md`.
