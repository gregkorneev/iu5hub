...
- Нижняя навигация — отдельный persistent functional layer над содержимым, не часть header или footer. Общая glass-поверхность содержит contextual action row, если текущий экран уже имел действия «Назад» или «К корню курса», и route links в нижнем ряду. Контекстные действия остаются кнопкой возврата/ссылкой в корень, без изменения обработчиков; selection bubble принадлежит только route links. CSS grid создаёт равные сегменты по фактическому числу ссылок (3 для обычного пользователя с Profile, 4 для подтверждённого admin), поэтому индикатор перестраивается при смене числа пунктов и viewport без фиксированных pixel offsets. Контейнер ограничен шириной экрана и центрируется, а не растягивается на desktop. Controls сохраняют hit area не меньше 44×44 px; `aria-current="page"` сообщает выбранный destination независимо от bubble. Для движения действует глобальный `prefers-reduced-motion`.
...
**Навигационный инвариант:** сохранены `HashRouter`, все существующие маршруты и иерархия каталога. «Каталог» всегда открывает `/`; «Поиск» восстанавливает последний поисковый запрос, пока приложение смонтировано. Search-result provenance следует за внутренними папками, а прямые ссылки разрешаются по реальному URL. Contextual Back/«К корню курса» показаны в том же нижнем functional layer на экранах, где уже существовали; обработчики и destinations сохранены. Кнопка Back и Telegram BackButton по-прежнему используют `goBack`, отдельно от переключения вкладок. Подробности route matrix, role visibility и Apple reference см. `telegram.md`.
