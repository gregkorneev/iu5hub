...
| --- | --- | --- |
| Course | `id`, `title`, `publicUrl` | `publicUrl` — публичный Yandex share root; необязательный `rootPath` выбирает вложенную папку внутри share. |
...
Конфигурация MVP задаёт три `Course`, которые образуют кнопки первого экрана. Для вложенной публичной папки `rootPath` задаёт стартовый путь относительно share root; repository не добавляет его повторно к Yandex API путям, уже содержащим этот префикс. Всё ниже корня не описывается вручную в UI: подпапки показываются как вложенные каталоги, а файлы — как материалы.

Для статической реализации допустимо versioned-конфигурацией хранить публичные root URL/идентификаторы, optional nested root paths и заранее подготовленный снимок дерева. Это не является live-синхронизацией: чтобы новые файлы появлялись сами, repository должен уметь получить актуальный список содержимого корневой папки и дочерних папок.
