...
- Mobile viewport scroll regression: Home, Profile and Schedule are checked at 390×844 and 320×568 with Telegram viewport/safe-area events. The document remains fixed; Home/Profile fit without main overflow at the standard size; Schedule shows two full lesson cards with more fixture lessons available through the named, keyboard-focusable list. Tests assert independent scrolling, no horizontal overflow, and a gap above the fixed navigation. The 320×568 Today mode also keeps both cards above the bar; low-height Week/group selection may use an internal main fallback. This is browser emulation; physical Telegram iOS/Android is not covered.
