...
- Final layout QA safe-area regression applies a 24px Telegram top inset and verifies first Profile and Schedule controls remain below the header at 320×568 and 390×844 in Chromium and WebKit. Schedule tests also confirm that the two visible lessons still fit in the schedule viewport and that portrait/landscape scroll remains confined to lessons.
- Week schedule date-window regression verifies exactly seven Moscow-time dates from today −3 days through today +3 days, today selected and centered on open, stable boundaries when another day is selected, and restoration of today after switching back from «Сегодня».
...
- First-launch welcome: `welcome.spec.ts` checks three intro steps, keyboard access, Skip/Start/Escape, localStorage persistence after reload, unchanged direct hash routes, Telegram BackButton pause/resume, dark Telegram theme, 320×568 safe-area fit, and axe accessibility/color contrast in Chromium, mobile Chromium, and WebKit.
...
2026-09-25: bottom navigation icon pass uses the centralized existing four destinations. Regression checks decorative SVG accessibility, icon-above-label alignment, ≥44px targets, no 320px overflow, and single-bubble containment for every selected tab; Chromium/mobile Chromium/WebKit 15/15. Schedule and Profile compact-height checks passed after setting the control height to 46px. Production screenshot: `/tmp/iu5hub-icon-tabs-final/bottom-navigation-persiste-ba31f-ed-footer-for-visual-review-mobile-chromium/bottom-bar-capsule.png`.

2026-09-25: Home scroll regression follow-up. Reproduced the stale internal `main` scroll at 295×667 (596px content in a 349px viewport) and found earlier cascading rules that allowed inner Home scrolling. The Home-only compact layout and greeting spacing are now checked with Telegram 24px top/bottom safe insets, short/long names, and 295×667, 320×568, 360×667, 390×720, 390×730, 390×844. Chromium/mobile Chromium/WebKit all pass the Home regression; independent dark-theme Chromium + WebKit review passed all 16 combinations at 295×667, 320×568, 360×667, and 390×720. Document and `main` scrollHeight equal their visible client heights, forced scrolling remains at zero, all homepage sections/footer/tabs fit without overlap or text clipping. See screenshots under `/tmp/iu5hub-home-postfix-*` and Playwright `test-results/home-links-*/`.
2026-09-26: Home spacing regression passes in Chromium, mobile Chromium and WebKit at viewports up to 430×932. Schedule browser fixtures pin the page clock to 12:30 Moscow time, keeping the synthetic current lesson stable across late-night CI runs; this affects tests only, not application schedule logic. `npm run qa` passed: lint, typecheck, 24 Vitest, 54 Node tests, all generated-data checks, build and 132/132 Playwright tests.
