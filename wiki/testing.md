...
- Semester-folder names are recognized in both `sem` and `семестр` forms, regardless of case; mobile regression checks the two-column tile layout and confirms even `Парадигмы и конструкции языков программирования` stays inside its tile and breaks only at spaces.
