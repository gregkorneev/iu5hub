...
export const test = base.extend<{ telegram: TelegramState; welcomeComplete: boolean }>({
  ...
  page: async ({ page, welcomeComplete }, use) => {
    ...
    await page.clock.setFixedTime(new Date(`${today}T09:30:00.000Z`))
    const todayUtc = new Date(`${today}T00:00:00Z`)
    const alternateDay = new Date(todayUtc.getTime() - 86400000).toISOString().slice(0, 10)
    ...
    await page.route('**/api/profile/schedule-group', async (route) => {
      ...
      const body = route.request().postDataJSON() as { groupId: string | null }
      scheduleGroupId = body.groupId
      return route.fulfill({ json: { groupId: scheduleGroupId, updatedAt: new Date().toISOString() } })
    })
